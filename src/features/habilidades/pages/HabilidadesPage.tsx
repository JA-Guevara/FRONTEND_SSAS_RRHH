...
export function HabilidadesPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try { if (editing) await actualizarHabilidad(editing.id, data, company?.id); else await crearHabilidad(data, company?.id); setEditing(null); setMessage('Habilidad guardada correctamente.'); await load() } catch (cause) { setError(cause instanceof Error ? cause.message : 'No se pudo guardar la habilidad.') }
  }
